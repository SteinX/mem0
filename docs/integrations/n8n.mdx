...
**User ID** and **Agent ID** are speaker scopes. When both are supplied, they combine with **OR** because default extraction stores each fact under the speaker that stated it:

```json
{ "OR": [{ "user_id": "alice" }, { "agent_id": "support-bot" }] }
```

**App ID** and **Run ID** are mandatory boundaries. When present, they remain outside the speaker OR under **AND**. Use a tenant-specific App ID when different users share an agent and must remain isolated.
...
- **Searching with User ID and Agent ID returns both speakers**: this is expected. Add a tenant-specific **App ID** or a **Run ID** to narrow the result set without hiding one speaker's inferred memories.
