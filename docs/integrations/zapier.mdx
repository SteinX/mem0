...
| **App ID** | | Scopes every extracted memory to an app or tenant |
...
| **User ID** | | Whose memories to search |
| **Agent ID** | | Also recall facts attributed to this agent speaker |
| **App ID** | | Require results to belong to this app or tenant |
| **Run ID** | | Require results to belong to this session or run |
...
| Field | Required | Purpose |
| --- | --- | --- |
| **User ID** | | Whose memories to list |
| **Agent ID** | | Also list facts attributed to this agent speaker |
| **App ID** | | Require results to belong to this app or tenant |
| **Run ID** | | Require results to belong to this session or run |
...
When both **User ID** and **Agent ID** are supplied, Mem0 recalls either speaker because default extraction stores each fact under the speaker that stated it. **App ID** and **Run ID** stay outside that speaker OR as mandatory boundaries. Use a tenant-specific App ID when different users share the same agent.

For Add, Search, and Get Memories, supply at least one of **User ID**, **Agent ID**, **App ID**, or **Run ID**. Each field is individually optional so app-only, run-only, and agent-only workflows remain valid.
