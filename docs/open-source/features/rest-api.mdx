...
Per-user keys inherit the creating user's scope. `GET /auth/me` accepts the
key and returns the owning user plus a safe `credential` descriptor:

```json
{
  "id": "3e7a1c81-4c3b-4bfa-bff8-248a02b8724d",
  "name": "Admin",
  ...
  "credential": {
    "kind": "core_api_key",
    "id": "bbfef730-0bc5-49c4-a6b8-6460192c93dd",
    "label": "codex-devbox",
...
The descriptor never includes the full key or its password hash. Its `kind`
identifies how the request was authenticated:

| `kind` | `id` | `label` | `key_prefix` |
|---|---|---|---|
| `session` | `null` | `null` | `null` |
| `core_api_key` | API key UUID | Configured label | First 12 characters |
| `operator_static` | `null` | `Legacy admin API key` | `null` |
| `disabled` | `null` | `null` | `null` |

Client API keys are data-plane credentials. Even when their owner is an
admin, they receive `403` from account mutation, API-key management, and
store-wide inventory routes such as `GET /entities`. Use an admin dashboard
session or the legacy operator credential for those control-plane operations.
...
#### Sidecar mutation recovery lookup

The self-hosted sidecar uses the private `_mem0_sidecar_mutation_id` query
parameter on `GET /memories` to reconcile an add whose HTTP result was
ambiguous. The value must be a lowercase 64-character hexadecimal marker. Add
at least one `user_id`, `agent_id`, or `run_id` filter whenever possible; the
server normalizes those identifiers and requires every returned memory to
match both the entity scope and marker exactly.

This lookup returns the raw `{"results": [...]}` list used for reconciliation,
including marker metadata needed by the sidecar. It does not change normal
entity-only `GET /memories` behavior. Authenticated callers can use a scoped
lookup. Omitting all entity filters makes it a store-wide inventory operation,
which rejects named client API keys and requires an admin session or legacy
operator credential.

```bash
curl -G http://localhost:8888/memories \
  -H "X-API-Key: $ADMIN_API_KEY" \
  --data-urlencode "user_id=alice" \
  --data-urlencode "_mem0_sidecar_mutation_id=<64-lowercase-hex>" \
  --data-urlencode "top_k=1000" \
  --data-urlencode "show_expired=true"
```
...
| `GET` | `/auth/me` | Get the authenticated user and safe credential descriptor |
| `PATCH` | `/auth/me` | Update the caller's name or email. Named client keys receive 403; duplicate email returns 409 |
...
All `/api-keys` endpoints require an admin session or legacy operator
credential. Non-admin sessions and named client API keys receive `403`.

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api-keys` | List the caller's API keys |
| `POST` | `/api-keys` | Create a new key; labels are trimmed and must contain 1–255 characters (422 otherwise); the full `m0sk_...` value is returned once |
...
All `/entities` endpoints require an admin session or legacy operator
credential. Non-admin sessions and named client API keys receive `403`.
