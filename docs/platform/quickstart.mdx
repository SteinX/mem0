...
For long-running TypeScript processes, `identityCacheMax` controls how many API-key identities are cached per process. It defaults to `50`; set it to a non-negative integer, or `0` to disable identity caching.
</Step>
